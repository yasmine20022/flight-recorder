...
def send_notification(user: str, message: str) -> dict:
    ...
    recipient email and ``message`` is the notification text. In production this would send
    a real email; here it appends to a local log. The replay engine blocks this tool.
    """
    # Defensive guard: never notify a fabricated address. The recipient must be a real
    # user the agent looked up via get_user_info. This makes the tool robust to the LLM
    # inventing placeholder emails — it gets rejected and must retry with the real one.
